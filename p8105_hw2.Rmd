...
**description:**
In the mci_merge data set, I did an inner_join of the baseline and amyloid data set by variable `id` to ensure that only that appear in both datasets are retained. After merging, there are `r nrow(mci_merge)/5` participants with `n rcol(mci_merge)` varibles left in the study. 
